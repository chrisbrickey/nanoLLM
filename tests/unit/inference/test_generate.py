...
@pytest.fixture
def mock_tokenizer_config() -> MagicMock:
    tok = MagicMock()
    tok.encode.return_value = [END_TOKEN_ID]
    tok.decode.return_value = "decoded text"
    config = MagicMock(spec=TokenizerConfig)
...
class TestGenerateTextOutput:
    def test_returns_decoded_string(
        ...
        mock_tokenizer_config.tokenizer.decode.assert_called_once()
    ...
    def test_decode_receives_start_tokens_plus_generated(
        ...
        decoded_args = mock_tokenizer_config.tokenizer.decode.call_args[0][0]
